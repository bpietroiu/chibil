...
- Lists a binary's native imports with `--print-imports` — function P/Invoke stubs and
  data imports grouped by native library, a link map for auditing the native dependency
  surface. (It flagged QuickJS's `qjsc_repl`/`qjsc_repl_size` as unresolved data imports,
  which led to bootstrapping `repl.c`; see [CompileQuickJS.md](CompileQuickJS.md) §3.)
...
- **QuickJS** (`quickjs-2025-09-13`) — Bellard's JS engine; all 7 interpreter
  ...
  `RegExp`, `Map`/`Set`, `BigInt`, `Proxy`, and the async/`Promise` job queue. The
  precompiled-bytecode REPL (`repl.c`) is generated by a `qjsc` bootstrapped with chibil
  itself, so the engine links with no unresolved data imports.
